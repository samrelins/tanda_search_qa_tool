...
class CordResultSummarizer:
    ...
    def summary_table(self, pop_keywords=default_pop_keywords):
        ...
        solution_answers = self._return_answers(
            question="what is the solution or improves",
            texts_dict=self.full_texts
        )

        print(f"Building table entries", flush=True)
        for cord_uid in tqdm(self.full_texts.keys()):
            ...
            summary_table["addressed_population"].append(
                find_populations(paper_text, pop_keywords)
            )
